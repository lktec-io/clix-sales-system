...
function CustomerDetail() {
  const { t, i18n } = useTranslation(['customers', 'sales', 'returns', 'common']);
  const { id } = useParams();
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 mb-5">
        <KPICard icon={FiShoppingBag} label={t('customers:detail.totalOrders')} value={customer.totalOrders} />
        <KPICard icon={FiDollarSign} label={t('customers:detail.totalSpent')} value={customer.totalSpent} formatter={(v) => formatCurrency(v)} />
        <KPICard icon={FiRotateCcw} label={t('customers:detail.totalReturns')} value={customer.totalReturns} />
      </div>
