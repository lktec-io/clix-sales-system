...
function drawFullHeader(doc, { config, dateRangeLabel, company, generatedByName, filtersLabel, branchLabel }) {
  ...
  const metaParts = [];
  metaParts.push(`Generated: ${new Date().toLocaleString('en-TZ', { dateStyle: 'medium', timeStyle: 'short' })}`);
  if (generatedByName) metaParts.push(`Prepared By: ${generatedByName}`);
...
function drawFooters(doc) {
  ...
  for (let i = 0; i < range.count; i += 1) {
    ...
    doc.fontSize(8).font('Helvetica').fillColor(COLOR_MUTED)
      .text('Generated by Clix Sales System', PAGE_MARGIN, bottom, { width: CONTENT_WIDTH, align: 'center', lineBreak: false });
    doc.fontSize(8).font('Helvetica').fillColor(COLOR_MUTED)
