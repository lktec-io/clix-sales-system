...
// Clix brand palette — Navy / Emerald / Gold, matching the app's own
...
// Navy/Green alternation only — matches the spec's "Navy Blue, Green,
// Light Gray, Gold accents (very subtle) — avoid excessive colors." Gold
// stays reserved for the one header divider below, not spread across
// every KPI card equally.
const KPI_ACCENTS = [COLOR_NAVY, COLOR_EMERALD];
...
function drawFullHeader(doc, { config, dateRangeLabel, company, generatedByName, filtersLabel, branchLabel, locale }) {
  ...
  doc.fontSize(8).font('Helvetica-Bold').fillColor(COLOR_MUTED)
    .text(company?.company_name || 'CLIX SALES SYSTEM', textLeft, headerTop, { width: CONTENT_WIDTH - (textLeft - PAGE_MARGIN) });
  ...
  doc.fontSize(17).font('Helvetica-Bold').fillColor(COLOR_NAVY)
    .text(`${config.title} ${t(locale, 'common.report')}`, textLeft, doc.y + mm(1.5), { width: CONTENT_WIDTH - (textLeft - PAGE_MARGIN) });
  ...
  const metaParts = [];
  metaParts.push(`${t(locale, 'common.generated')}: ${new Date().toLocaleString(dateLocale, { dateStyle: 'medium', timeStyle: 'short' })}`);
  if (generatedByName) metaParts.push(`${t(locale, 'common.preparedBy')}: ${generatedByName}`);
  if (branchLabel) metaParts.push(`${t(locale, 'common.branch')}: ${branchLabel}`);
  metaParts.forEach((line) => doc.text(line, { width: CONTENT_WIDTH }));
  if (filtersLabel) {
    doc.text(`${t(locale, 'common.filters')}: ${filtersLabel}`, { width: CONTENT_WIDTH });
  }
...
function drawCompactHeader(doc, { config, company, locale }) {
  doc.fontSize(9).font('Helvetica-Bold').fillColor(COLOR_NAVY)
    .text(company?.company_name || 'Clix Sales System', PAGE_MARGIN, PAGE_MARGIN - mm(6), { continued: true, width: CONTENT_WIDTH / 2 });
  doc.font('Helvetica').fillColor(COLOR_MUTED)
    .text(`   ·   ${config.title} ${t(locale, 'common.report')}`, { continued: false });
...
function drawFooters(doc, locale) {
  ...
  for (let i = 0; i < range.count; i += 1) {
    ...
    doc.fontSize(8).font('Helvetica').fillColor(COLOR_MUTED)
      .text(t(locale, 'common.generatedBy', { company: 'Clix Sales System' }), PAGE_MARGIN, bottom, { width: CONTENT_WIDTH, align: 'center', lineBreak: false });
    doc.fontSize(8).font('Helvetica').fillColor(COLOR_MUTED)
      .text(t(locale, 'common.pageOf', { page: i + 1, total: range.count }), PAGE_MARGIN, bottom + mm(4.5), { width: CONTENT_WIDTH, align: 'center', lineBreak: false });
  }
}
...
function drawFinancialSummary(doc, financialSummary, locale) {
  sectionTitle(doc, t(locale, 'common.financialSummary'));
  const rows = [
    [t(locale, 'financialSummary.totalRevenue'), formatCell('totalRevenue', financialSummary.totalRevenue)],
    [t(locale, 'financialSummary.averageDailySales'), formatCell('totalRevenue', financialSummary.averageDailySales)],
    ...(financialSummary.averageInvoice != null ? [[t(locale, 'financialSummary.averageInvoice'), formatCell('totalRevenue', financialSummary.averageInvoice)]] : []),
    [t(locale, 'financialSummary.highestSalesDay'), `${financialSummary.highestSalesDay.date} — ${formatCell('totalRevenue', financialSummary.highestSalesDay.value)}`],
    [t(locale, 'financialSummary.lowestSalesDay'), `${financialSummary.lowestSalesDay.date} — ${formatCell('totalRevenue', financialSummary.lowestSalesDay.value)}`],
  ];
...
function drawBarChart(doc, title, rows) {
  ...
  data.forEach((row, index) => {
    ...
    // One consistent color, not a rotation — these bars are the same
    // metric over time, not distinct categories that need telling apart,
    // and a single color reads calmer/more premium ("avoid excessive
    // colors") than a red/green/gold-per-bar chart would.
    doc.rect(x, y, barWidth, barHeight).fill(COLOR_NAVY);
...
export function buildReportPdf(type, report, { dateFrom, dateTo, company, generatedByName, filtersLabel, branchLabel, locale = 'en' } = {}) {
  ...
  const headerContext = { config, dateRangeLabel, company, generatedByName, filtersLabel, branchLabel, locale };
  ...
  if (report.summary && config.summaryLabels) {
    sectionTitle(doc, t(locale, 'common.executiveSummary'));
  ...
  if (report.financialSummary) {
    drawFinancialSummary(doc, report.financialSummary, locale);
  }
  ...
  if (Array.isArray(chartRows) && chartRows.length > 0) {
    drawBarChart(doc, t(locale, type === 'profit' ? 'common.dailyProfitTrend' : 'common.salesTrend'), chartRows);
  }
  ...
  if (Array.isArray(report.financialSummary?.monthlyTrend)) {
    ...
    drawBarChart(doc, t(locale, 'common.monthlyTrend'), monthlyRows);
  }
  ...
  // so they are not run through the dictionary here; they render as
  // authored (English) regardless of locale. See reportAnalysis.js for the
  // full rationale.
  if (Array.isArray(report.analysis) && report.analysis.length > 0) {
    sectionTitle(doc, t(locale, 'common.businessAnalysis'));
  ...
  if (Array.isArray(report.recommendations) && report.recommendations.length > 0) {
    sectionTitle(doc, t(locale, 'common.recommendations'));
  ...
  config.breakdowns.forEach(({ key, title: breakdownTitle, labelHeader }) => {
    ...
    if (!rows || rows.length === 0) {
      doc.fontSize(9).font('Helvetica').fillColor(COLOR_MUTED).text(t(locale, 'common.noDataForFilters'), { width: CONTENT_WIDTH });
      doc.fillColor(COLOR_NAVY);
      divider(doc);
      return;
    }
    ...
    if (totalColumn) {
      ...
      doc.text(t(locale, 'common.grandTotal'), PAGE_MARGIN + 2, rowY, { width: columnWidth * totalColIndex - 4 });
